    ...
    def run(self, modifications=None, send_discord=False, target_num=None, workflow_type='default', workflow=None):
        try:
            if modifications is not None:
                ...
                wf_config = self.settings.get("workflow_types", {}).get(self.active_workflow, {})
                output_enabled = wf_config.get("output_enabled", True)
                if output_enabled:
...
class ComfyCategorizer(ComfyBridge):
    ...
    def draw(self, prompt, neg="", seed=-1, node_id=None, workflow_type=None):
        ...
        # アクティブなワークフローを更新（先に active_workflow を設定してから load_settings を呼ぶ）
        ...
        # self.workflowfile からワークフローファイルを読み込む
        ...
        for node_info in self.get_dual_prompt_encoder_nodes(current_workflow):
            ...
            if neg_assign is True:
                ...
                negative_prompt = neg.strip() if neg else ""
    ...
    def _save_active_workflow(self):
        ...
        try:
            # 他プロセスの変更を考慮し、ファイルを先に読み込んでから active_workflow のみ上書き
            if os.path.exists(self.settings_path):
                with open(self.settings_path, 'r', encoding='utf-8') as f:
                    self.settings = json.load(f)
            else:
    ...
    def run(self, modifications=None, send_discord=False, target_num=None, workflow_type='default', workflow=None):
        try:
            ...
            if workflow_type and workflow_type != self.active_workflow:
                self.active_workflow = workflow_type
                ...
                for node_info in self.get_dual_prompt_encoder_nodes(workflow):
                    ...
                    if neg_assign is True:
                        ...
                        for cat in [c.strip() for c in neg_assign.split(",")]:
                            if cat == "all":
                                for c, items in self.categories.items():
                                    if c != "nsfw": neg_parts.extend([i.strip() for i in items if i.strip()])
                            else:
    ...
    for node_info in target_nodes:
        ...
        if class_type == "DualPromptEncoder":
            ...
            if nid in nodes_by_id:
                existing_config = nodes_by_id[nid]
        ...
        if class_type == "DualPromptEncoder":
            ...
            # 用途確認
            current_purpose = "positive"
            if nid in nodes_by_id:
                current_purpose = nodes_by_id[nid].get("purpose", "positive")
            ...
            if is_negative:
                ...
                current_text = init.get("text", "")
                if nid in nodes_by_id and nodes_by_id[nid].get("assign"):
                    current_text = nodes_by_id[nid]["assign"]
                ...
                default_val = input("  default []: ").strip()
                if nid in nodes_by_id:
                    default_val = default_val or nodes_by_id[nid].get("default", "")
...
if __name__ == "__main__":
    ...
    if not is_wizard:
        # workflow_type が指定された場合、それを active_workflow として保存
        if workflow_type:
            cc.active_workflow = workflow_type
            cc._save_active_workflow()
            cc.load_settings()
